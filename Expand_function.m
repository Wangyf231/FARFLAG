function exp_array = Expand_function(node_x, node_y, hn, xTarget, yTarget, CLOSED, MAX_X, MAX_Y, mesh_point)
    ...
    for i = 1:size(directions, 1)
        s_x = node_x + directions(i, 1);
        s_y = node_y + directions(i, 2);
        ...
        if (s_x > 0 && s_x <= MAX_X) && (s_y > 0 && s_y <= MAX_Y) % node within array bound
            ...
            if flag == 1
                ...
                exp_array(exp_count, 3) = hn + distance(node_x, node_y, s_x, s_y, mesh_point); % cost of travelling to node
                exp_array(exp_count, 4) = distance(xTarget, yTarget, s_x, s_y, mesh_point); % distance between node and goal
    ...
    end % End of directions for loop
end
